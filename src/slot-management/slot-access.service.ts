...
import { dayRange, isCalendarDay, todayIn } from './slot-schedule';

/** Who is managing slots, and for which business. */
export interface SlotContext {
  user: Users;
  business: Business;
  /** Owners manage every staff member's slots; employees only their own. */
  isOwner: boolean;
}

@Injectable()
export class SlotAccessService {
  constructor(
   ...
   */
  async resolve(currentUser: ActiveUserData): Promise<SlotContext> {
    const user = await this.usersService.findActiveUser(currentUser.sub);
    if (user.role === Role.Client) {
      throw new ForbiddenException('Only business staff can manage slots');
    }
   ...
   */
  async resolveStaff(ctx: SlotContext, staffId?: number): Promise<Users> {
    if (!staffId || staffId === ctx.user.id) {
      return ctx.user;
    }
    if (!ctx.isOwner) {
      throw new ForbiddenException('Employees can only manage their own slots');
    }
  ...
  /**
   * Employees only ever see their own slots; owners see everyone's unless
   * they filter by `staffId`.
   */
  visibleStaffId(ctx: SlotContext, staffId?: number): number | undefined {
