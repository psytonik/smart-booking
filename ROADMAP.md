...
- **Booking platform for service businesses** (barbers, salons, clinics, tutors, …). Every master works their own way. **MVP scope, revised 2026-09-23: one vertical first — barbers + beauty — in Israel.** Other verticals (salons, clinics, tutors) come after the pilot proves out; the domain model (Service/Staff/Booking) already generalizes, so this is an onboarding/GTM sequencing decision, not a schema change.
...
- **Surfaces, revised 2026-09-23 — web-first, no mandatory app.** The client books through the business's **web page — no install required**. A native mobile app for clients comes *after* the pilot, as a retention surface for repeat clients (favorites, rebooking, push, reminders) — not a prerequisite for the first booking. Business side: web for businesses (a business app may follow). Web itself is **two separate front-ends**: a **business admin/dashboard app** (owner/staff, behind auth) and a **marketing site** (public: landing pages, business discovery/public pages, pricing — SEO-driven, no auth). Each gets its own deploy pipeline and can move independently; they share only the API.
- **Notifications, revised 2026-09-23 — WhatsApp moves up, not "later," but as a click-to-chat gate, not the full Business API.** Priced out 2026-09-23: WhatsApp Business API (Meta Cloud API) needs per-business Meta verification, pre-approved message templates, and binding the business's number to the API (conflicts with normal app use on that number outside Meta's newer, still-limited coexistence mode) — real operational overhead per business, even though the per-message cost itself is cheap (~$0.005/utility message in Israel). For the pilot, ship a **`wa.me/<business phone>?text=...` deep link** instead: a "Message on WhatsApp" button in confirmation emails and on the booking page, opening a chat with the business's own number, pre-filled with the booking details. No Meta integration, no template approval, no automated proactive sends. The full Business API (real automated WhatsApp reminders, sent by the platform) becomes a **later feature, built only if there's demand for it** — see G7b in the roadmap. Telegram and mobile push stay later too, once the client app exists.
- **"Smart"** refers to future intelligent features in the client app (e.g. suggestions, reminders).
- **Differentiator, revised 2026-09-23: not the AI-generated design.** The AI-generated business page is a V2 nice-to-have, not the reason a business pays. The actual value proposition is automation that cuts manual work: fewer WhatsApp back-and-forths, fewer empty slots, fewer no-shows, automatic rebooking prompts. Every business still gets an exclusive AI-generated page for its page eventually — just not as the MVP pitch.
...
4. **Notification channels**: turn `NotificationsService` into a channel-agnostic dispatcher (email now; **a WhatsApp click-to-chat link pulled forward to the pilot** — not an adapter, just a generated `wa.me` URL with the business's own number, no send-side integration; Telegram, mobile push, and a real WhatsApp Business API adapter stay later, demand-gated) with per-user contact details and preferences. The queue already fits this.
5. **Mobile-ready API**: versioned routes (`/v1`), because shipped apps can't be force-updated; device tokens for push; stable error format. **Not required for the MVP booking flow** (that's web, no app) — this is groundwork for the post-pilot client app.
6. **AI-designed business pages** *(V2, not MVP — see product direction above)*: the AI produces a **design spec (JSON)**, not HTML: palette, typography, layout variant, section styles, copy, image references. Web and the mobile apps render it with their own components. Reasons: one spec renders natively on iOS/Android and web; no third-party HTML/JS (no XSS, no broken layouts); the spec can be validated before publishing (contrast, required blocks like the booking button); owners can pick between variants and tweak them; versions allow rollback. Generation runs as a background job on the existing queue; images need object storage (GCS).
...
### Milestone 7 — Israel pilot (MVP), revised 2026-09-23

**Strategy shift:** the earlier plan sequenced AI design and a mandatory client app ahead of monetization/retention mechanics. Revised after a product-strategy review: ship a narrow, web-first pilot for one vertical, prove activation/retention, *then* build AI design, the client app and discovery. See the PRD's Monetization and Release-stages sections for the full reasoning.

- [ ] **G7a WhatsApp click-to-chat gate (pulled forward).** Not the Business API — a generated `wa.me/<business phone>?text=...` link, shown as a "Message on WhatsApp" button in confirmation emails and on the booking page, opening a chat with the business's own WhatsApp number pre-filled with the booking details. Zero Meta integration: no verification, no template approval, no number binding. Ships *with* the pilot, since WhatsApp is the channel that matters in Israel. Does not enable automated proactive reminders — see G7c.
- [ ] **G12 Business QR code.** Each business gets a generated QR code (its public page / booking link) that clients can scan — printable, for mirrors/reception/business cards. Primary acquisition loop for the pilot.
...
- [ ] **G9 Subscriptions.** Plans, per-business subscription, limits (staff count, …), billing provider. Needs a product decision on plans first. Pricing should sell automation (fewer no-shows, less manual admin), not the AI design — see product direction.
- [ ] **Pilot rollout.** 20–50 businesses, barbers + beauty vertical, Israel. Tracked against the PRD's activation definition (signup → service → staff → hours → first real client booking), time-to-first-booking, and week-4 retention — not just activation %. No new features added mid-pilot; the pilot's job is to validate what's already built.

### Later (post-pilot)

- [ ] **G8 Mobile-ready API.** `/v1` prefix, device-token registration for push, documented error format. Groundwork for the client app below — not needed for the pilot's web-only booking flow.
- [ ] **Mobile client app.** Native app for clients, positioned as a *retention* surface (favorites, one-click rebooking, reminders, push, discover-nearby) once there's a base of repeat clients from the pilot — not a prerequisite for a first booking.
- [ ] **F1 Nearby search / discovery.** Public search, filters, categories. Deferred until enough businesses are on the platform for discovery to return good results — see Phase F below for the open rules questions. Until then the platform is SaaS-only: businesses bring their own clients via QR/link (G12/G13).
- [ ] **G11 AI-designed business pages** *(V2 — lower priority than G7a/G12/G13/G9 above; not a launch differentiator)*.
...
- [ ] **G7b Telegram + mobile push.** Later notification adapters, once the client app exists.
- [ ] **G7c WhatsApp Business API (real automated reminders), demand-gated.** Upgrades G7a from a click-to-chat link to actual server-sent WhatsApp messages (proactive reminders, confirmations) via Meta's Cloud API or a BSP (Twilio/360dialog/etc.). Only build this if the pilot shows businesses actually want automated WhatsApp reminders badly enough to justify the operational cost: per-business Meta Business verification, message templates submitted for Meta approval, and binding the business's number to the API (which conflicts with normal personal/app use of that number, aside from Meta's still-limited coexistence mode). Priced 2026-09-23: the per-message cost itself is cheap (~$0.005 per utility message in Israel, proactive/out-of-window; free within an open 24h service window) — the real cost is the per-business onboarding friction, not the API bill.
...
- [ ] **Other verticals** (salons, clinics, tutors, …), once the barbers + beauty pilot validates activation/retention. The domain model already generalizes; this is GTM sequencing, not a schema change.
...
- [ ] **Nearby business search / discovery** *(post-pilot — see "Later" under Milestone 7)*. Coordinates are geocoded and stored in `Location` but never used. Deferred on purpose: the pilot is SaaS-only (businesses bring their own clients via QR/link), and discovery needs enough businesses on the platform to return good results.
